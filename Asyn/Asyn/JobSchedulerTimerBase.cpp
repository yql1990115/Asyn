...
CJobSchedulerTimerBase::CJobSchedulerTimerBase(const char* strName) 
	: m_window(this, &CJobSchedulerTimerBase::EventProc, &CJobSchedulerTimerBase::TimerProc, &CJobSchedulerTimerBase::OnException)
{
	m_strName = strName ? strName : "";
}

CJobSchedulerTimerBase::~CJobSchedulerTimerBase()
{
	Shutdown();
}

/// 定时调度器名称
const char* CJobSchedulerTimerBase::Name() const
{
...
*/
BOOL CJobSchedulerTimerBase::Startup()
{
	BOOL l_bRet = FALSE;
	//
...
*/
void CJobSchedulerTimerBase::Shutdown(DWORD dwMilliSecondsTimeOut)
{
...
*/
BOOL CJobSchedulerTimerBase::HasBeenTerminated() const
{
...
请求暂停调度程序执行
*/
void CJobSchedulerTimerBase::Stop()
{
	//nothing
}
...
请求恢复调度程序执行
*/
void CJobSchedulerTimerBase::Resume()
{
	//nothing
}
...
*/
BOOL CJobSchedulerTimerBase::HasBeenSuspended() const
{
	//nothing
	return FALSE;
}
...
判断任务调度程序是否还活着
*/
BOOL CJobSchedulerTimerBase::HasAlive() const
{
...
void CJobSchedulerTimerBase::NotifyExit()
{
	m_terminated.Set();
}
...
*/
BOOL CJobSchedulerTimerBase::Schedule(IJob* pJob)
{
	assert(pJob);
	//
...
如果作业放入调度池中失败, 将调用作业的 \ref Release() 函数释放作业并返回FALSE
*/
BOOL CJobSchedulerTimerBase::ScheduleTimer(DWORD dwSecondDelay, BOOL bRepeat, IJob* pJob)
{
	assert(pJob);
	//
	BOOL l_bSchedule = FALSE;
...
*/
BOOL CJobSchedulerTimerBase::KillTimer(IJob* pJob)
{
	BOOL l_bKill = FALSE;
	CCriticalSectionLock l_lock(m_CacheTimerJobLock);
	auto l_it = m_CacheTimerJob.find(pJob);
...
void CJobSchedulerTimerBase::EventProc()
{
	LIST_JOB l_listJob;
	//
	{
...
void CJobSchedulerTimerBase::TimerProc()
{
...
long CJobSchedulerTimerBase::OnException(struct _EXCEPTION_POINTERS * pInfo)
{
	//assert(0);	//异常中断中不使用断言
...
/// 移除定时作业链表
void CJobSchedulerTimerBase::RemoveListTimerJob()
{
	LIST_TIMER_JOB l_listTimerJob;
	//
	{
...
/// 移除定时作业链表
void CJobSchedulerTimerBase::RemoveListTimerJob(LIST_TIMER_JOB& listTimerJob)
{
...
/// 移除定时作业链表中可以被移除的作业
void CJobSchedulerTimerBase::RemoveListTimerJobIfCanKill(LIST_TIMER_JOB& listTimerJob)
{
...
/// 移除定时作业缓存 
void CJobSchedulerTimerBase::RemoveCacheTimerJob()
{
...
/// 移除定时作业缓存中可以被移除的作业 
void CJobSchedulerTimerBase::RemoveCacheTimerJobIfCanKill()
{
	CCriticalSectionLock l_lock(m_CacheTimerJobLock);
...
/// 处理定时作业
void CJobSchedulerTimerBase::OnProcessTimerJob(CTimerJobHelper* pTimerJob)
{
	assert(pTimerJob);
	DWORD l_dwEx = 0;
	__try
...
/// 移除作业链表
void CJobSchedulerTimerBase::RemoveListJob()
{
	LIST_JOB l_listJob;
	//
	{
...
/// 移除作业链表
void CJobSchedulerTimerBase::RemoveListJob(LIST_JOB& listJob)
{
...
/// 处理作业
void CJobSchedulerTimerBase::OnProcessJob(IJob* pJob)
{
	assert(pJob);
	DWORD l_dwEx = 0;
	__try
...
*/
BOOL CJobSchedulerTimerBase::SignalHandle()
{
